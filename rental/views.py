...
class UserProfileViewSets(viewsets.ModelViewSet):
    ...
    permission_classes = [
        rest_permissions.IsAuthenticated,
        permissions.UserProfileOwnerUpdate,
    ]
    ...
    def get_object(self):
        queryset = self.filter_queryset(
            models.UserProfile.objects.get(user__username=self.kwargs.get("user"))
        )
        return queryset
    ...
    @action(
        ...
        url_name="borrow",
        url_path="borrow",
        description="List Books Borrowed by User",
        permission_classes=[permissions.IsOwner,],
    )
    def borrowed_books(self, request, **kwargs):
        borrowed = self.filter_queryset(
            models.Borrowed.objects.filter(user__username=self.kwargs.get("user"),)
        )
...
class PublisherViewSets(viewsets.ModelViewSet):
    ...
    def publisher_list_books(self, request, **kwargs):
        books = self.filter_queryset(
            models.Book.objects.filter(publisher=self.kwargs.get("pk"))
        )
...
class AuthorViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        queryset = self.filter_queryset(
            models.Author.objects.get(author__username=self.kwargs.get("obj_username"))
        )
        return queryset

    @action(
        detail=True, url_name="books", url_path="books",
    )
    def author_book_list(self, request, **kwargs):
        ...
        books = models.Book.objects.filter(
            author__author__username=self.kwargs.get("obj_username")
        )
        serialized_books = serializers.BookDetailedSerializer(books, many=True)
        return Response(serialized_books.data, status=status.HTTP_200_OK)
...
class BorrowedViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        borrowed_book_count = models.Borrowed.objects.filter(
            user=serializer.validated_data["user"], is_returned=False
        ).count()
        error_msg = [
            "The user package ('%s') not allowing to rent more than %s book(s)"
            % (profile.user_type, profile.book_limit),
            "Current rented books are : %s"
            % (
                models.Borrowed.objects.get(
                    user=serializer.validated_data["user"], is_returned=False
                )
            ),
        ]
        if profile.book_limit == borrowed_book_count:
            raise rest_serializers.ValidationError(
                detail=error_msg, code=status.HTTP_405_METHOD_NOT_ALLOWED
            )
...
class GenreViewSet(viewsets.ModelViewSet):
    ...
    def list_books(self, request, **kwargs):
        books = self.filter_queryset(
            models.Book.objects.filter(genre=self.kwargs.get("pk"))
        )
        serialized_books = serializers.BookDetailedSerializer(books, many=True)
        return Response(serialized_books.data, status=status.HTTP_200_OK)
